...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            \item[(a)] $\sin(x)\in\Omega^\times$: Given that $\int_{-\infty}^{\infty}|\sin(x)|^2 dx$ diverges (since integral of $\sin^2(x)$ diverges), then $\sin(x)\notin H$; however, since $-1\leq \sin(x)\leq 1$, any $\phi(x)\in \Omega$ also is converging at a fast enough rate (which satisfies $\int_{-\infty}^{\infty}\phi(x)dx$ converges), then one has $\int_{-\infty}^{\infty}\sin(x)\phi(x)dx$ also converges (by the boundedness of $\sin(x)$). As a result, $\sin(x)\in \Omega^\times$.
            ...
            \item[(c)] It's clear that $\int_{-\infty}^{\infty}|x^2\cos(x)|^2 dx$ diverges (since $\lim_{x\rightarrow\infty}x^4\cos(x)$ diverges), so $x^2\cos(x)\notin H$, hence also $x^2\cos(x)\notin \Omega$ either. However, for any $\phi(x)\in \Omega$, one has $x^2\phi(x)\in \Omega$ by the result in part 2, then $\int_{-\infty}^{\infty}x^2\cos(x)\phi(x)dx = \int_{-\infty}^{\infty}\cos(x)\left(x^2\phi(x)\right)dx$ also converges (since again $-1\leq \cos(x)\leq 1$, which it is bounded; and, for any $\psi(x)\in \Omega$, it's converging fast enough so $\int_{-\infty}^{\infty}\psi(x)dx$ also converges).
            
            As a result, $x^2\cos(x)\in\Omega^\times$.
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \end{align}
        Which, in general there isn't a good way of breaking the functions into sums of these values.
