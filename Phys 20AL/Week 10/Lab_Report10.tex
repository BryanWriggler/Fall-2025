...
\begin{figure}[h!]
    \centering
    \includegraphics[width=80mm]{lab_setup10.png}
    \caption{Picture of the actual Lab Setup}
    \label{fig:lab_setup}
\end{figure}

\subsection{Mirage Setup}
...
\hline
\end{tabular}
\caption{Focal Length for Concave Lens (unit: mm)}
...
\hline
\end{tabular}
\caption{Light Source and Image Distances for Convex and Concave Lenses (unit: mm)}
\label{tab:lens_eq}
\end{table}
...
\subsection{Transversal Magnification}
The definition of Transversal Magnification is $M_T := \frac{s_i}{s_o}$. Which, with $\delta s_i$ and $\delta s_o$ be the uncertainty respectively (based on the measurement of the ruler, it'll be up to $\pm 0.5$ mm), one has the following propogation of uncertainty:
\begin{align}
    \delta M_T = \sqrt{\left(\frac{\partial M_T}{\partial s_i}\delta s_i\right)^2 + \left(\frac{\partial M_T}{\partial s_o}\delta s_o\right)^2} = \sqrt{\left(\frac{\delta s_i}{s_o}\right)^2+\left(\frac{s_i}{s_o^2}\delta s_o\right)^2}
\end{align}
Then, using the data in Table \ref{tab:lens_eq}, for Convex 1 (with focal length $f=81.167$ mm) and the Concave Lense (with focal length $f=-98.413$ mm), one has the following magnification calculation:
\begin{table}[h!]
\centering
\begin{tabular}{l||cc|cc}
\hline
\textbf{Measured Focal Length (mm)} & \textbf{$s_o$ (mm)} & \textbf{$s_i$ (mm)} & \textbf{$M_T = s_i/s_o$} & \textbf{$\delta M_T$} \\
\hline
\multicolumn{5}{l}{\textbf{Convex 1 (f = 81.167 mm)}} \\
 & 200.000 & 228.000 & -1.140 & 0.004 \\
 & 250.000 & 187.000 & -0.748 & 0.002 \\
 & 300.000 & 167.000 & -0.557 & 0.002 \\
 & 350.000 & 154.000 & -0.440 & 0.002 \\
 & 400.000 & 148.000 & -0.370 & 0.001 \\
\hline
\multicolumn{5}{l}{\textbf{Concave (f = -98.413 mm)}} \\
 & 250.000 & -107.000 & 0.428 & 0.002 \\
 & 300.000 & -104.000 & 0.347 & 0.001 \\
 & 350.000 & -101.000 & 0.289 & 0.001 \\
 & 400.000 & -98.000 & 0.245 & 0.001 \\
 & 450.000 & -96.000 & 0.213 & 0.001 \\
\hline
\end{tabular}
\caption{Transversal Magnification $M_T$ with respect to Light Source Distance $s_o$ and Image Distance $s_i$}
\label{tab:trans_magnify}
\end{table}

Which, one can observe that the propogation of uncertainty in general is small in comparison to the magnification, hence one has a high confidence to conclude that the values can represent the transversal magnification in each case.

\hfil

\subsection{Mirage Setup}

For Mirage Setup, some data are collected as follow:
\begin{figure}[h!]
    \centering
    \includegraphics[width=100mm]{mirage data.jpg}
    \caption{The Radius of Curavture, Focal Length, and Height of the mirrors}
    \label{fig:mirage_data}
\end{figure}

Where, under the small angle approximation, one can assume $f\approx \frac{r}{2}$, showing $f \approx 84.333$ mm (based on the given height and other lenggth of the mirror). With height $h \approx 43$ mm, one can say the focal length $f \approx 2h$ (two times the height of the mirror). So, if one has two identical mirrors, and stack them in the way where the mirror sides face each other, one would have the focal point of each mirror being approximately right on the other mirror's surface (and at the center) as follow:
\begin{figure}[h!]
    \centering
    \includegraphics[width=100mm]{mirage explain.jpg}
    \caption{Illustration of the Mirage Setup}
    \label{fig:mirage_explain}
\end{figure}

Since the object (blue) is right on the top mirror's focal point (botto red dot), then the light from the object after reflected by the top mirror, becomes a parallel light.

Now, the parallel light gets reflected by the bottom mirror, and concentrated toward the bottom mirror's focal point (the top red dot), as a result when the light gets observed by human eyes, the object has the light passes through (and concetrated) at the top focal point, forming an image at the top focal point. In reality it looks like the object is floating on the top opening.

\newpage

\section{Error Analysis}

Within this experiment, besides the human measurement mistakes (or human limitation of making the measurement), here are some potential sources of error:
\begin{enumerate}
    \item The light source isn't necessarily a perfect point source or parallel light, which will cause some uncertainty in the experimental behavior, and affect the final measurements.
    \item The lens itself is also not perfect (just approximately the structure used for lenses), which such imperfection could also cause some uncertainty in the final measurements.
\end{enumerate}
...
\section{Conclusion}
In this lab, several observations and measurements are made:
\begin{enumerate}
    \item For focal length measurements, the three convex lenses have average focal lengths being 81.167 mm, 161.667 mm, and 203.000 mm; the concave mirror has average focal length $-58.500$ mm; and, the concave lens has average focal length -98.413 mm. These data all have small standard deviation (uncertainty), which has a high confidence that these average focal length is a good representative of the actual focal length.
    
    \hfil

    \item For the lens equation, unfortunately each measured light source distance $s_o$ and image distance $s_i$ has reciprical sum $\frac{1}{s_o}+\frac{1}{s_i}$ with a high percent error when comparing to the inverse focal length $\frac{1}{f}$, which the collected data has a low confidence of supporting the lens equation, which the lens equation is not a good model of representing this experimental data.
    
    \hfil

    \item For the transversal magnification, the data are all provided in Table \ref{tab:trans_magnify}, which one can observe that the propogation of uncertainties $\delta M_T$ are all relatively small in compareson to the magnification $M_T$, hence there is a high confidence that the calculated magnification represnts the actual experimental situation.
\end{enumerate}

Some possible errors in the experiment include: the imperfection of light source (from being a point source or a parallel source), and the imperfection of lens shape (in general having some errors), both have a possibility of causing uncertainties in the measured image distance / behavior of light with focal lengths.
