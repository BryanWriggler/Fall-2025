...
\end{itemize}
For potential improvement related to these two problems, a light source with stronger intensity and concentration could help (but also need to consider the budget for the experimental equipments). Also, based on the equation, with $\frac{\partial\lambda}{\partial L}=\frac{L}{\sqrt{L^2+x^2}}-1\approx -1.015\cdot 10^{-7}$, and $\frac{\partial\lambda}{\partial x}=\frac{x}{\sqrt{L^2+x^2}}\approx  4.505\cdot 10^{-4}$, also with $\delta L=0.02$ m, and $\delta x=5\cdot 10^{-4}$ m, the propogation of uncertainty is given as follow:
\begin{align}
...
\end{align}
One can see that in comparison to $\lambda\approx 5.631\cdot 10^{-7}$ m, the uncertainty is still quite large, so this data doesn't have that well of a confidence of representing the model provided.

\hfil

Besides the human limitation, some potential sources of errors include:
\begin{itemize}
    \item The transparent materials (the half-cylinder, triangular prism, etc.) may have non-uniform density, potentially 
\end{itemize}
