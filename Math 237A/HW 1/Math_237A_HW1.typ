...
#problem[
  ...
  Now, based on the above lemma, all $f in ker(phi)$ can be decomposed into $(y-x^2)dot f_1+(z-x^3)dot f_2+f_3(x)$ for some $f_1,f_2 in k[x,y,z]$ and $f_3(x) in k[x]$. Then, plugin to $phi$ we get:
  ...
  Based on #text(weight: "bold")[Proposition 1.7] in Hartshorne, given $Y$ as an affine algebraic set, its dimension is the same as the Krull Dimension of its coordinate ring $k[Y]$. Since here $k[Y] tilde.equiv k[t]$, with $k[t]$ being a PID, then $(0)$ is a prime ideal, while any other nonzero prime ideal $P subset.eq k[t]$ is maximal. Hence, the dimension of $k[t]$ is $1$ (since the largest strictly increasing chain of prime ideal is $(0) subset.neq P$ for nonzero prime ideal $P subset.eq k[t]$, due to the maximality of $P$). Hence, $Y$ is in fact an algebraic variety of dimension 1, and this finishes all the proof.
]
...
#problem[
  ...
  However, this contradicts one of the previous conclusions that $AA^1$ is Hausdorff. Hence, the initial assumption must be false, showing that Zariski Topology on $AA^2$ is not the same as product topology of $AA^1 times AA^1$ (given $AA^1$ is equipped with its own Zariski Topology).
