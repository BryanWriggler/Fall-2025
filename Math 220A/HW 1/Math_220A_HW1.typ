...
#problem[
  ...
  + For definiteness, given $f:X arrow.r Z$ and $g:Y arrow.r Z$ two abelian group homomorphisms, and say $g$ is surjective. Then, let the fiber product $X times_Z Y <= X times Y$ collect all the element $(x,y) in X times Y$ satisfying $f(x)=g(y)$ (together with $p_1:X times_Z Y arrow.r X$ and $p_2:X times_Z Y arrow.r Y$ be the two projections). 
    ...
    To show that $p_1$ (the pull-back of $g$) is also surjective, consider the following: For all $z in Z$, by surjectivity of $g$ shows that there exists $y in Y$, such that $g(y)=z$, 
]
...
#problem[
  ...
  + Given any $k:W arrow.r Z$, let $f:X arrow.r W$ and $g:Y arrow.r W$ be two morphisms contained in $"Mor"_Z(X,W)$ and $"Mor"_Z(Y,W)$ respectively. 

  Suppose $l:P arrow.r Z$ is an object in $cal(C)_Z$, together with morphisms $p_1:P arrow.r X$ and $p_2:P arrow.r Y$ in $"Mor"_Z(P,X)$ and $"Mor"_Z(P,Y)$ respectively serve as fiber product of $f,g$ (i.e. $f compose p_1=g compose p_2:P arrow.r W$, and such property is final). Then, the functor $F$ gets the following:
  $ F(P) = "Mor"_Z (T,P), quad F p_1 = p_1 compose \_:"Mor"_Z (T,P) arrow.r "Mor"_Z (T, X) $
  $ F p_2=p_2 compose \_:"Mor"_ Z (T,P) arrow.r "Mor"_Z (T,Y) $
]