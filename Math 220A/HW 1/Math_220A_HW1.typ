...
#problem[
  ...
  Where the above equation uses the fact that $g$ is in the center, and $h$ commutes with itself. Yet, this shows that $k k'=k'k$ for arbitrary $k,k' in G$, which $G$ is abelian, contradicting our initial assumption. Hence, the assumption is wfalse, $G$ with prime square power must be abelian.
...
#problem[
  ...
  + Here, consider a regular $n$-gon, then its rotation by radian $(2pi)/n$ counterclockwise (denote as $sigma$) has order $n$, and a reflection along one of the symmetric axis passing through the $1^upright(s t)$ vertex (labeled as $tau$) has order $2$, so $sigma^n=tau^2=e$. 

    Then, since every of the symmetry on regular $n$-gon must send consecutive vertices to consecutive vertices, $sigma tau$ send vertices $1$ and $2$ to $1$ and $n$ ($tau$ as reflection), then send vertex $1$ and $n$ to vertices $2$ and $1$ (so it swaps the two vertices).

    For $tau sigma^(n-1)$, they first send vertices $1$ and $2$ to vertices $n$ and $1$ (by $sigma^(n-1)$), then send vertices $n$ and $1$ to $2$ and $1$ (by $tau$). Hence, $sigma tau$ and $tau sigma^(n-1)$ provides the same permutation on the vertices (since knowing the relation between $2$ of the vertices immediately determines the other vertices, due to the property of isometry on $n$-gon). Hence, $sigma tau=tau sigma^(n-1)$ (since they provides the same permutation on the vertices).
  ...
  + Given that $mat(0,-1;1,0)^2=-id$, then $mat(0,-1;1,0)$ has order $4$; also, since $zeta$ is a primitive $n$-th root of unity, then $zeta^n =  zeta^(-n)=1$ (while any integer $0<k<n$ doesn't satisfy this relation). Hence, $mat(zeta,0;0,zeta^(-1))$ has order $n$ since its power $k$ has the form $mat(zeta^k,0;0,zeta^(-k))$). So, if $D_(4n)$ is finite, then $4$ and $n$ (order of the two generators) must divide $|D_(4n)|$, hence $lcm(4,n)=4n$ divides $|D_(4n)|$ (since $n$ is odd).
    ...
    The reason why $D_(4n)$ is finite, is because if consider the multiplication of the two generators, we get:
    $ mat(0,-1;1,0)mat(zeta,0;0,zeta^(-1))=mat(0,-zeta^(-1);zeta,0) = mat(zeta^(-1),0;0,zeta)mat(0,-1;1,0)=mat(zeta,0;0,zeta^(-1))^(-1)mat(0,-1;1,0) $
    if consider the subgroup $H$ generated by $mat(zeta,0;0,zeta^(-1))$, the above relation shows that it's normal (since both generators has the conjugation of elements in $H$ staying in $H$). Also, since it provides that the two generators can swap position, with the cost that the one with primitive $n$-th root is inverted), then every group element $sigma in D_(4n)$ in fact can be written as $mat(zeta,0;0,zeta^(-1))^k mat(0,-1;1,0)^l$ for some $k,l in ZZ$. Since there are at most $n$ distinct elements for $mat(zeta,0;0,zeta^(-1))^k$ and there are at most $4$ distinct elements for $mat(0,-1;1,0)^l$, then there are at most $4n$ distinct elements in $D_(4n)$.
