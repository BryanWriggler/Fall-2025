...
\begin{proof}
    ...
    Let $X$ denotes the random variable about the number of cartons to open before finding one without rotten eggs, which $X \in\{0,1,...,100\}$ (since there are total of $100$ cartons). 

    Notice that for most of them, the probability for a chosen carton to contain no rotten egg is given as $q=\frac{\begin{pmatrix}1000\\12\end{pmatrix}}{\begin{pmatrix}1200\\12\end{pmatrix}}\approx 0.1109$. Which, since the number of eggs are large enough, can treat the probability of each carton having no rotten eggs aproximately being $q$. In other words, the probability for one carton to contain some rotten eggs, is given by $p = 1-q \approx 0.8891$.

    Hence, for each $i\in\{0,1,...,100\}$, the probability of opening up exactly $i$ cartons with rotten eggs, then the $(i+1)^\text{th}$ being non-rotten, is given as follow $\PP(X=i) = p^i \cdot (1-p)$. Hence, the expected value for the number of cartons to open before finding one without rotten eggs, is approximately given as follow:
    \begin{align}
        \EE[X] \approx \sum_{i=0}^{100}i \cdot \PP(X=i) = sum_{i=0}^{100}i\cdot p^i \cdot (1-p) \approx 8.0147
    \end{align}

    \begin{comment}
    The goal is to calculate the probability that $\PP(X=i)$ (or number of cartons to open being $i$, before finding one without rotten eggs). Since for each event $I=j$ for distinct $j$ are disjoint, hence by law of total probability, we get:
    \begin{align}
        \PP(X= i) &= \sum_{j=0}^{100}\PP(X= i | I=j)\PP(I=j)
    \end{align}
    Given that there are $I=j$ cartons without rotten eggs, then the probability for the pick to be exactly $i$ before having one that's not rottened, is given by: 
    \begin{align}
        \PP(X=i|I=j) = \begin{cases}
            0 & i > 100-j\\
            \frac{\begin{pmatrix}100-j\\i\end{pmatrix}\begin{pmatrix}j\\1\end{pmatrix}}{\begin{pmatrix}100\\i+1\end{pmatrix}} & i\leq 100-j
        \end{cases}
    \end{align}
    For $i>100-j$, since $100-j$ denotes the number of cartons with rotten eggs, it's impossible to pick out $i>100-j$ cartons with rotten eggs. For $i\leq 100-j$, it's choosing $i$ cartons out of the $100-j$ ones with rotten eggs, choose $1$ out of the $j$ cartons without rotten eggs, out of the general ways of taking $i+1$ cartons out of $100$ ones.

    \hfil

    Finally, for $\PP(I=j)$, we first need to pick out $12 j$ out of $1000$ not rottened eggs to put into these $j$ cartons (without rotten eggs), out of the ones in general where just randomly picking $12 j$ out of $1200$ eggs. Hence, it's with the following probability:
    \begin{align}
        \PP(I=j) = \frac{\begin{pmatrix}1000\\12j\end{pmatrix}}{\begin{pmatrix}1200\\12j\end{pmatrix}}
    \end{align}
    For this, we need $12j\leq 1000$, showing that $j\leq 83$. Hence, the expected value is given as follow:
    \begin{align}
        \EE[X] = \sum_{i=0}^{100}i \cdot \PP(X=i) = 
    \end{align}
    \end{comment}
\end{proof}
...
\begin{proof}
    ...
    \begin{itemize}
        \item[a)] When the balls are returned each time, since there are total of $N+M$ balls, with $N$ being green balls, then the probability of getting green balls in each take is $p=\frac{N}{N+M}$, while getting red balls in each take is $q=\frac{M}{N+M}=1-p$.
        
        Given any $i\in\NN$, for the event $X=i$ to happen, we need to get $i$ green balls consecutively, while on the $(i+1)^\text{th}$ draw, we need to get red ball to end the process. Hence, $\PP(X=i) = p^i \cdot q^1 = \left(\frac{N}{N+M}\right)^i \cdot \frac{M}{N+M}$. So, the expected value of $X$, $\EE[X]$ is given as follow:
        \begin{align}
            \EE[X] &= \sum_{i=0}^{\infty}i\cdot \PP(X=i) = \sum_{i=0}^{\infty}\left(\frac{N}{N+M}\right)^i\cdot\frac{M}{N+M} = \frac{M}{N+M}\cdot\frac{1}{1-\frac{N}{N+M}} = \frac{M}{N+M}\cdot \frac{N+M}{M}=1
        \end{align}
        So, the expected number of green balls being drawn is $\EE[X]=1$.

        \hfil

        \item[b)] If the removed balls are not putting back to the urn, then people can draw at most $N$ green balls out of the urn, or $X\in\{0,1,...,N\}$. For any $i\in\{0,1,..,n\}$, in case for the event $X=i$ to happen, the person needs to consecutively draw $i$ green balls out of the urn, and on the $(i+1)^\text{th}$ move one needs to draw a red ball. 
        
        \vspace{0.7em}

        Here, we'll consider the probability of drawing $0\leq i\leq N$ consecutive green balls (without considering how to end with red balls) using induction.
        
        For $i=1$, the probability of drawing out $i=1$ green ball is given by $\frac{N}{N+M}$. For $i=2$, the probability of drawing out 2 green balls consecutively is given by $\frac{N}{N+M}\cdot \frac{N-1}{N+M-1}$ (since the second green ball is picked from $N-1$ remaining green balls, while there are $N+M-1$ balls in total remaining). 
        
        Inductively, suppose for given integer $0\leq k<i$, the probability of drawing out $k$ green balls consecutively is given by $\frac{N!}{(N-k)!}\cdot \frac{(N+M-k)!}{(N+M)!}$, then for the case of $k=i$, since the probability of drawing the first $(i-1)$ green balls is given by $\frac{N!}{(N-(i-1))!}\cdot \frac{(N+M-(i-1))!}{(N+M)!}$, while there are $N-(i-1)$ green balls, and $N+M-(i-1)$ total balls remaining, hence drawing the $i^\text{th}$ green ball now has a probability of $\frac{N-(i-1)}{N+M-(i-1)}$. Hence, the probability of drawing $k=i$ consecutive green balls, is given by $\left(\frac{N!}{(N-(i-1))!}\cdot \frac{(N+M-(i-1))!}{(N+M)!}\right)\cdot \frac{N-(i-1)}{N+M-(i-1)} = \frac{N!}{(N-i)!}\cdot \frac{(N+M-i)!}{(N+M)!}$.

        \vspace{0.7em}

        Finally, if given integer $0\leq i\leq N$, we know the event $X=i$ happens if $i$ consecutive green balls are drawn (which has probability $\frac{N!}{(N-i)!}\cdot \frac{(N+M-i)!}{(N+M)!}$), while the $(i+1)^\text{th}$ balll drawn is red. Since after the first $i$ picks, there are $M$ red balls leftover, while only $N+M-i$ total balls remaining, the probability of drawing a red ball on the $(i+1)^\text{th}$ term is $\frac{M}{N+M-i}$. Hence, probability of $X=i$ is given by:
        \begin{align}
            \PP(X=i)=\frac{N!}{(N-i)!}\cdot\frac{(N+M-i)!}{(N+M)!}\cdot \frac{M}{N+M-i}
        \end{align}
        As a result, the corresponding expected value for $X$ is given as follow:
        \begin{align}
            \EE[X] = \sum_{i=0}^{N}i\cdot \PP(X=i) = \sum_{i=0}^{N}i\cdot \frac{N!}{(N-i)!}\cdot\frac{(N+M-i)!}{(N+M)!}\cdot \frac{M}{N+M-i}
        \end{align}
    \end{itemize}

    After all the above calculation, here are the numerical result for $N=7$, $M=5$:
    \begin{itemize}
        \item[a)] If it's a sample with replacement, then $\EE[X] = 1$ without ambiguity.
        \item[b)] If it's a sample without replacement, then $\EE[X]$ using the formula above, is given as follow:
        \begin{align}
            \EE[X]_{N=7, M=5} = \sum_{i=0}^{7}i\cdot \frac{7!}{(7-i)!}\cdot\frac{(12-i)!}{12!}\cdot\frac{5}{12-i} \approx 1.1667
        \end{align}
    \end{itemize}
\end{proof}
