...
\section*{3 (tweak the justification)}
\begin{ques}\label{q3}
    8.4.8:
...
\begin{proof}
    ...
    \begin{align}
        ...
        &= \left(\frac{1}{\sqrt{2}\cdot\sqrt{2\pi}}e^{-\frac{(u/\sqrt{2})^2}{2}}\right)\left(\frac{1}{\sqrt{2}\cdot\sqrt{2\pi}}e^{-(v/\sqrt{2})^2/2}\right)
    \end{align}
    Since the function can be written as $f(u) \cdot g(v)$ Here $f(t)=g(t) = \frac{1}{\sqrt{2}\cdot\sqrt{2\pi}}e^{-(t/\sqrt{2})^2/2}$), showing that the joint probability density of $U,V$ is separable in terms of the variables $u$ and $v$, hence $U,V$ are independent.
\end{proof}
...
\begin{proof}
    First, let $X$ be the random variable 
\end{proof}
